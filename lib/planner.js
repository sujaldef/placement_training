...
import ytDataBundled from '@/lib/data/ytdata.json';
import dsaSheetBundled from '@/lib/data/dsasheet.json';
import indiaBixBundled from '@/lib/data/indiabix.json';
...
function loadJson(fileName) {
  const bundledByName = {
    'ytdata.json': ytDataBundled,
    'dsasheet.json': dsaSheetBundled,
    'indiabix.json': indiaBixBundled,
  };

  const bundled = bundledByName[fileName];
  ...
  if (!existingPath) {
    throw new Error(`Planner source file not found: ${fileName}`);
  }
  ...
  try {
    ...
    throw new Error(`Invalid JSON in planner source: ${fileName}. ${message}`);
  }
}
...
function generateDays(settings, source) {
  ...
  const safeVideos = asArray(safeSource.videos);
  const safeDsaTopics = asArray(safeSource.dsaTopics);
  const safeAptitudeTopics = asArray(safeSource.aptitudeTopics);
  const safeDsaByVideoPosition =
    safeSource.dsaByVideoPosition instanceof Map
      ? safeSource.dsaByVideoPosition
      : new Map();
  ...
  const videoStartIndex = Math.max(
    0,
    safeVideos.findIndex(
      (video) => video.position >= settings.startVideoPosition,
    ),
  );

  let videoCursor = clampIndex(videoStartIndex, safeVideos.length);

  let dsaCursor = 0;
  if (settings.startDsaTopicId) {
    const located = safeDsaTopics.findIndex(
  ...
  if (settings.startAptitudeId) {
    const foundAptitude = safeAptitudeTopics.findIndex(
  ...
  while (inRange <= endDate && dayCount < 250) {
    ...
    while (videoCursor < safeVideos.length && dayVideos.length < 5) {
      const currentVideo = safeVideos[videoCursor];
      const videoSeconds =
    ...
    for (const video of dayVideos) {
      ...
      const videoTopics = [];
      const linkedTopics = safeDsaByVideoPosition.get(video.position) || [];

      for (const topic of linkedTopics) {
        const topicIndex = safeDsaTopics.findIndex(
      ...
      while (videoTopics.length < 2 && dsaCursor < safeDsaTopics.length) {
        const fallbackTopic = safeDsaTopics[dsaCursor];
        dsaCursor += 1;
    ...
    if (!dayVideos.length) {
      ...
      while (standaloneAdded < 3 && dsaCursor < safeDsaTopics.length) {
        const topic = safeDsaTopics[dsaCursor];
        dsaCursor += 1;
    ...
    if (aptitudeEnabled && safeAptitudeTopics.length > 0) {
      const aptitudeTopic =
        safeAptitudeTopics[aptitudeCursor % safeAptitudeTopics.length];
      aptitudeCursor += 1;
...
function buildWeeks(days) {
  ...
  for (let index = 0; index < days.length; index += 7) {
    ...
    const weekEnd = fromIsoDate(lastDay.isoDate);
...
export function getPlannerData(inputSettings = {}) {
  ...
  const safeWeeks = buildWeeks(asArray(days));

  return {
    settings,
    weeks: asArray(safeWeeks),
    meta: {
      ...
      dsaTopics: asArray(source.dsaTopics).map((topic) => ({
        id: topic.id,
        topic: topic.topic,
        step: topic.step,
        linkedVideoPosition: topic.linkedVideoPosition,
      })),
      aptitudeTopics: asArray(source.aptitudeTopics).map((topic) => ({
        id: topic.id,
        topic: topic.topic,
